  ...
  main.variable(observer("chart")).define("chart", ["d3","width","height","nodes","groups","forceCluster","forceCollide"], function(d3,width,height,nodes,groups,forceCluster,forceCollide)
{
  ...
  var interval = d3.interval(timer, 25);
  
  return svg.node()
}
);

  main.variable(observer("chart2")).define("chart2", ["d3","width","height","nodes2","groups2","forceCluster2","forceCollide"], function(d3,width,height,nodes2,groups2,forceCluster2,forceCollide)
{
  // Variables.
  var time_so_far = 0;
  // The SVG object.
...
function forceCluster() {
  ...
  main.variable(observer("forceCluster2")).define("forceCluster2", ["groups2"], function(groups2){return(
function forceCluster2() {
  const strength = 2;
  let nodes2;
  ...
  function force(alpha) {
    ...
    for (const d of nodes2) {
      d.vx -= (d.x - groups2[d.group].x) * l;
      d.vy -= (d.y - groups2[d.group].y) * l;
    }
  }
  force.initialize = _ => nodes2 = _;
